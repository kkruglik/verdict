...
fn parse_constraint_between_two_numbers() {
    let result = parse_column_constraint("between", &json!([0, 100])).unwrap();
    assert!(matches!(
        result,
...
fn parse_constraint_is_in_integers() {
    let result = parse_column_constraint("is_in", &json!([1, 2, 3])).unwrap();
    assert!(matches!(
        result,
...
fn parse_constraint_contains_string() {
    let result = parse_column_constraint("contains", &json!("foo")).unwrap();
    assert!(matches!(result, ColumnConstraint::Contains(s) if s == "foo"));
}

#[test]
fn parse_constraint_length_between() {
    let result = parse_column_constraint("length_between", &json!([1, 50])).unwrap();
    assert!(matches!(
        result,
...
fn parse_constraint_errors_on_unknown_name() {
    let result = parse_column_constraint("nonexistent", &json!(null));
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
